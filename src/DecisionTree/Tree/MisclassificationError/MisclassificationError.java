 ...
 * 
 */
package DecisionTree.Tree.MisclassificationError;
...
public class MisclassificationError {
	...
	public Node constructDecisionTree(String[][] trainData, String[] trainTruth){
		boundaryHashMap=new BinarySplitMisclassificationError().createBoundaryHashMap(trainData, trainTruth);
	...
	private Node createDecisionTree(int[][] trainingData, int[] trainingTruth, HashSet<Integer> attributesToBeVisited, HashSet<Integer> samplesToBeVisited){
		int attributeWithMaxInfoGain=getAttributeWithMinError(trainingData, trainingTruth, attributesToBeVisited, samplesToBeVisited);
	...
	private int getAttributeWithMinError(int[][] trainingData, int[] trainingTruth, HashSet<Integer> attributesToBeVisited, HashSet<Integer> samplesToBeVisited){
		if(attributesToBeVisited.size()==0) return -1;
		else{
			double error=Double.MAX_VALUE;
			int attributeWithLeastGain=-1;
			for(int i:attributesToBeVisited){
				double ig=getError(trainingData, trainingTruth, i, samplesToBeVisited);
	...
	private double getError(int[][] trainingData, int[] trainingTruth, int attribute, HashSet<Integer> samplesToBeVisited){
		...
		double d= calculateError(count1Previous, count0Previous, count1Rest, count0Rest);
		if(Double.isNaN(d)) System.out.println("NaN value found!");
		return d;
	}
	
	private double calculateError(int count1Previous,int count0Previous, int count1Rest, int count0Rest){
		/*double gini1=0.0;
		if(count1Previous!=0 || count0Previous!=0) gini1=calculateError(count1Previous, count0Previous);
		double gini2=0.0;
		if(count1Rest!=0 || count0Rest!=0) gini2=calculateError(count1Rest, count0Rest);
		int val1=count1Previous+count0Previous;
		int val2=count1Rest+count0Rest;
		return val1*gini1+val2*gini2;*/
		
		int count1=count1Previous+count1Rest;
		int count0=count0Previous+count0Rest;
		return calculateError(count1, count0);
	}
	
	private double calculateError(int count1, int count0){
