...
public class BinarySplitMisclassificationError {
	...
	private double getBoundaryUsingBinarySplit(){
		...
		for(int i=0;i<attributeValue.size();i++){
			...
			if(i!=attributeValue.size()-1 && attributeValue.get(i)!=attributeValue.get(i+1) && classValue.get(i)!=classValue.get(i+1)){
				...
				double tempInformationGain=calculateError(count1SoFar, count0SoFar, totalOnes-count1SoFar, totalZeros-count0SoFar);
